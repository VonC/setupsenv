...
%_task% "Must check if '%APPDATA%\JetBrains\IdeaIC2022.3\idea.properties' exist"
if exist "%APPDATA%\JetBrains\IdeaIC2022.3\idea.properties" (
    ...
    findstr /r /c:"org.bouncycastle.asn1.allow_unsafe_integer" "%APPDATA%\JetBrains\IdeaIC2022.3\idea.properties" >nul
    if errorlevel 1 (
        ...
        echo "org.bouncycastle.asn1.allow_unsafe_integer=true" >> "%APPDATA%\JetBrains\IdeaIC2022.3\idea.properties"
        if errorlevel 1 (
    ...
    %_info% "No file '%APPDATA%\JetBrains\IdeaIC2022.3\idea.properties' found"
    REM create the parent folder structure, then copy the file there
    mkdir "%APPDATA%\JetBrains\IdeaIC2022.3"
    if errorlevel 1 (
        %_fatal% "Unable to create '%APPDATA%\JetBrains\IdeaIC2022.3' folder" 1
    ) else (
        %_ok% "Folder '%APPDATA%\JetBrains\IdeaIC2022.3' created"
    )
    %_task% "Copy 'idea.properties' file to '%APPDATA%\JetBrains\IdeaIC2022.3'"
    copy /y "%script_dir%\custom\idea.properties" "%APPDATA%\JetBrains\IdeaIC2022.3\idea.properties" >nul
    if errorlevel 1 (
        %_fatal% "Unable to copy 'idea.properties' file to '%APPDATA%\JetBrains\IdeaIC2022.3'" 1
    ) else (
        %_ok% "'idea.properties' file copied to '%APPDATA%\JetBrains\IdeaIC2022.3'"
    )
)
