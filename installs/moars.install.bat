...
if not exist "%PRGS%\%prgs_folder%\%prg_folder%\moar.exe" (
    %_task% "Must clean up '%PRGS%\%prgs_folder%\%prg_folder%'"
    del /S "%PRGS%\%prgs_folder%\%prg_folder%\moar.exe"
    if not errorlevel 0 (
        %_fatal% "Unable to cleanup '%prg_folder%' in '%PRGS%\%prgs_folder%'" 82
    )
    %_ok% "'%PRGS%\%prgs_folder%\%prg_folder%' cleaned up"
    %_task% "Must copy '%PRGS%\%prgs_folder%\%prg_folder%.exe' to folder '%prg_folder%' as moar.exe"
    rem echo copy "%PRGS%\%prgs_folder%\%prg_folder%.exe" "%PRGS%\%prgs_folder%\%prg_folder%\moar.exe"
    copy "%PRGS%\%prgs_folder%\%prg_folder%.exe" "%PRGS%\%prgs_folder%\%prg_folder%\moar.exe" >nul 2>&1
    if errorlevel 1 (
        %_fatal% "Unable to copy 'moar.exe' in '%PRGS%\%prgs_folder%\%prg_folder%'" 83
    )
    %_ok% "'%PRGS%\%prgs_folder%\%prg_folder%.exe' copied to folder '%prg_folder%' as moar.exe"
) else (
    %_ok% "'%PRGS%\%prgs_folder%\%prg_folder%\moar.exe' already present"
)
