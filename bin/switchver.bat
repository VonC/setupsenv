...
set "standalone_switchver="
if "%script_dir%"=="" (
    set "standalone_switchver=true"
)
...
if exist %script_dir%\..\batcolors (
  ...
  echo "[switchver] batcolor not found in script_dir '%script_dir%'" >&2
  endlocal
  exit /b 1
)
...
if %count% equ 0 (
    %_fatal% "No %prg_version% version found in '%PRGS_ROOT%'" 3
)
