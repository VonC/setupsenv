...
for /f "tokens=* delims=-" %%a in ('ls -l "%PRGS%\yeds\current"') do ( set "yed_version=%%a" )
set "yed_version=%yed_version:*/yeds/yED-=%"
for /f "tokens=1 delims=_" %%a in ('echo %yed_version%') do ( set "yed_version=%%a" )
